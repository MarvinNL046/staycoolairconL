...
- maak `Asset-ID` deterministisch en route-gebaseerd, maar blokspecifiek: gebruik een lowercase kebab-id afgeleid van de concrete pagina/route plus `Bloktype` en een korte regionale invalshoek; hergebruik exact dezelfde `Asset-ID` als hetzelfde blok in een latere run of tweede canonieke rij terugkomt; gebruik nooit datums, random suffixes of vrijblijvende labels;
