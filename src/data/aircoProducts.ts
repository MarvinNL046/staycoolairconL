...
import { cashflowProductMatches } from './cashflowProductMatches';
import { cashflowArticleOffers } from './cashflowArticleOffers';
import { priceIncludingVat } from '../utils/installationPricing';
...
export interface AircoProduct {
  ...
  price?: number;
  cashflowPrice?: boolean;
  cashflowSourceSlug?: string;
}

const originalAircoProducts: AircoProduct[] = [
  {
...
export const aircoProducts: AircoProduct[] = originalAircoProducts.map(product => {
  const direct = cashflowArticleOffers[product.id];
  if (direct) {
    if (product.name !== direct.expectedName || product.type !== direct.expectedType) throw new Error('Gewijzigd artikel bij Cashflow-match: ' + product.id);
    return { ...product, price: priceIncludingVat(direct.netCents, direct.vatRate)/100, cashflowPrice: true, cashflowSourceSlug: direct.sourceSlug };
  }
  const match = cashflowProductMatches[product.id];
  if (!match) return { ...product, price: undefined, cashflowPrice: undefined };
  const variants = cashflowVariants.filter(v => v.family === match.family && v.configuration.startsWith(match.capacity + ' kW') && (!match.color || v.name.endsWith(match.color)));
  if (variants.length !== 1 || Number(product.capacity) !== Number(match.capacity.replace(',', '.')) || product.type !== 'single-split') throw new Error('Ongeldige Cashflow-match: ' + product.id);
  return { ...product, price: priceIncludingVat(variants[0].netCents,variants[0].vatRate)/100, cashflowPrice: true, cashflowSourceSlug: variants[0].sourceSlug };
});
